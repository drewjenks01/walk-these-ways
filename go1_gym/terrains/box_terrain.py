...
class BoxTerrain(Terrain):
    ...
    def initialize(self):
        ...
        # add walls
        self.wall_actors = []
        poses = [gymapi.Vec3(self.env.cfg.terrain.terrain_width * self.env.cfg.terrain.num_rows / 2, self.env.cfg.terrain.terrain_width * self.env.cfg.terrain.num_rows, self.env.cfg.terrain.terrain_length / 2),
                 gymapi.Vec3(self.env.cfg.terrain.terrain_width * self.env.cfg.terrain.num_rows / 2, 0, self.env.cfg.terrain.terrain_length / 2),
                 gymapi.Vec3(self.env.cfg.terrain.terrain_width * self.env.cfg.terrain.num_rows, self.env.cfg.terrain.terrain_width * self.env.cfg.terrain.num_rows / 2, self.env.cfg.terrain.terrain_length / 2),
                 gymapi.Vec3(0, self.env.cfg.terrain.terrain_width * self.env.cfg.terrain.num_rows / 2, self.env.cfg.terrain.terrain_length / 2)]
        oris = [gymapi.Quat(0.7068252, 0, 0, 0.7073883),
                gymapi.Quat(0.7068252, 0, 0, 0.7073883),
                gymapi.Quat(0, 0.7068252, 0, 0.7073883),
                gymapi.Quat(0, 0.7068252, 0, 0.7073883)]
        for i in range(4):
            pose = gymapi.Transform()
            pose.r = oris[i]
            pose.p = poses[i]
            wall_actor = self.env.gym.create_actor(env, self.base_asset, pose, None, collision_group, collision_filter)
            self.wall_actors.append(wall_actor)
            self.env.gym.set_actor_scale(env, wall_actor, self.env.cfg.terrain.terrain_length)
            self.env.gym.set_rigid_body_texture(env, wall_actor, 0, gymapi.MeshType.MESH_VISUAL, texture)
