...
class LCMAgent:
    ...
    def get_obs(self):
        ...
        if self.cfg["env"]["observe_only_lin_vel"]:
            ob = np.concatenate(
                (self.body_linear_vel.reshape(1, -1) * self.obs_scales["lin_vel"], ob),
                axis=1,
            )
        ...
        if "terrain" in self.cfg.keys() and self.cfg["terrain"]["measure_heights"]:
            ...
            heights = (
                np.clip(robot_height - 0.5 - self.measured_heights, -1, 1.0)
    ...
    def step(self, actions, hard_reset=False):
        ...
        images = {
            "front": self.se.get_camera_front(),
        ...
        for k, v in images.items():
            if images[k] is not None:
                images[k] = cv2.resize(
                    images[k],
                    dsize=(
                        images[k].shape[0] // downscale_factor,
                        images[k].shape[1] // downscale_factor,
                    ),
