    ...
    def prepare(self):
        ...
        for i in range(self.env.cfg.terrain.num_rows):
            for j in range(self.env.cfg.terrain.num_cols):
                ...
                if self.env.custom_heightmap is not None:
                    ...
                    if terrain_type <= cume_props[0]: # flat/rough
                        ...
                        if self.env.terrain_props is not None:
                            ...
                            step_height = (self.env.cfg.terrain.max_step_height - self.env.cfg.terrain.min_step_height) * difficulty + self.env.cfg.terrain.min_step_height
                        num_steps = self.env.cfg.terrain.num_steps
                        ...
                        if self.env.terrain_props is not None:
                            ...
                            step_run = (self.env.cfg.terrain.max_step_run - self.env.cfg.terrain.min_step_run) * torch.rand(1) + self.env.cfg.terrain.min_step_run
                            step_height = -1 * ((self.env.cfg.terrain.max_step_height - self.env.cfg.terrain.min_step_height) * difficulty + self.env.cfg.terrain.min_step_height)
                        num_steps = self.env.cfg.terrain.num_steps
                        for k in range(num_steps):
                            step_px = int(step_run * k / self.env.cfg.terrain.horizontal_scale)
                            if self.env.terrain.cfg.length_per_env_pixels - 1.0 / self.env.cfg.terrain.horizontal_scale  <= step_px * 2:
                        ...
                        platform_size = self.env.cfg.terrain.platform_size
                        ...
                        self.terrain_cell_center_heights[i, j] = 0
                    elif terrain_type <= cume_props[5]:  # tiles
                        # switch parameters to discrete units
                        tile_height = 0.20 * difficulty
                        max_height = int(tile_height / self.env.cfg.terrain.vertical_scale)
                        min_size = int(1.0 / self.env.cfg.terrain.horizontal_scale)
                        max_size = int(2.0 / self.env.cfg.terrain.horizontal_scale)
                        ...
                        for _ in range(30):
                            lx = np.random.choice(width_range)
                            ly = np.random.choice(length_range)
                            start_i = np.random.choice(range(0, width, 4)) + start_px
                            start_j = np.random.choice(range(0, length, 4)) + start_py
                            self.height_samples[start_i:start_i+lx, start_j:start_j+ly] = np.random.choice(height_range)

                        platform_size = self.env.cfg.terrain.platform_size
                        platform_size = int(platform_size / self.env.cfg.terrain.horizontal_scale / 2)
                        x1 = width // 2 - platform_size + start_px
                        x2 = width // 2 + platform_size + start_px
                        y1 = length // 2 - platform_size + start_py
                        y2 = length // 2 + platform_size + start_py

                        self.height_samples[x1:x2, y1:y2] = 0
                        self.terrain_cell_center_heights[i, j] = 0
                    elif terrain_type <= cume_props[6]:  # waves
                        amplitude = 0.2 * difficulty
                        num_waves = 2
                        width = end_px - start_px
                        length = end_py - start_py
                        div = width / (num_waves * np.pi * 2)
                        x = np.arange(0, width)
                        y = np.arange(0, length)
                        xx, yy = np.meshgrid(x, y, sparse=True)
                        xx = xx.reshape(width, 1)
                        yy = yy.reshape(1, length)
                        wave = torch.tensor(amplitude*np.cos(yy / div) + amplitude*np.sin(xx / div), dtype=torch.float32)
                        self.height_samples[start_px:end_px, start_py:end_py] += wave.to(self.env.device) / self.env.cfg.terrain.vertical_scale
                ...
                lin_x = np.linspace(0, self.env.cfg.terrain.terrain_length, self.env.terrain.cfg.width_per_env_pixels,
                                    endpoint=False)
                lin_y = np.linspace(0, self.env.cfg.terrain.terrain_width, self.env.terrain.cfg.length_per_env_pixels,
                                    endpoint=False)
                x, y = np.meshgrid(lin_x, lin_y)
                perlin_seed = i * self.env.cfg.terrain.num_cols + j
                self.height_samples[start_px:end_px, start_py:end_py] = self.height_samples[start_px:end_px, start_py:end_py] + torch.tensor(perlin(x, y, seed=perlin_seed, octaves=self.env.cfg.terrain.perlin_octaves, lacunarity=self.env.cfg.terrain.perlin_lacunarity, gain=self.env.cfg.terrain.perlin_gain), device=self.env.device) * float(roughness) / self.env.cfg.terrain.vertical_scale
