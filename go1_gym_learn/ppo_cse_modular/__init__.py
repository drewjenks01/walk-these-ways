...
class Runner:
    ...
    def __init__(self, env, device='cpu'):
        ...
        if RunnerArgs.resume:
            ...
            loader = ML_Logger(root="/scratch/gmargo/walk-these-ways-dev/runs/",
                               prefix=RunnerArgs.resume_path)
            print(loader.glob("*"))
            weights = loader.load_torch(f"checkpoints/{RunnerArgs.resume_checkpoint}")
        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            if it % RunnerArgs.save_interval == 0:
                ...
                #     logger.duplicate(f"checkpoints/ac_weights_{it:06d}.pt", f"checkpoints/ac_weights_last.pt")
                    ...
                    adaptation_module = copy.deepcopy(self.alg.actor_critic.estimation_module).to('cpu')
                    ...
                    # save the adaptation module buffers for estimator verification
                    torch.save(self.alg.actor_critic.estimation_module.privileged_obs_buffer, f"./tmp/legged_data/privileged_obs_buffer.pt")
                    torch.save(self.alg.actor_critic.estimation_module.obs_history_buffer, f"./tmp/legged_data/obs_history_buffer.pt")
                    wandb.save(f"./tmp/legged_data/privileged_obs_buffer.pt")
                    wandb.save(f"./tmp/legged_data/obs_history_buffer.pt")
